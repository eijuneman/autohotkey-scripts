...
SetWorkingDir, \\192.168.0.1\hdd2\일정표 최근\일정표220730
...
Gui, 1:Add, ListView, x20 y+0 w610 h230 vlv1 glv1 Grid hwndHLV1 Sort NoSortHdr -Multi BackgroundFFFCF3, 안보임|출하일|日|출고시각|업체명|출고지|컷|門|`%
Gui, 1:ListView, lv1
LV_ModifyCol(1, 0) ;
LV_ModifyCol(2, 65)  ;
LV_ModifyCol(3, 30)  ;
...
Gui, 1:Add, ListView, y+0 w610 h300 vlv2 glv2 Grid hwndHLV2 NoSortHdr Sort -Multi BackgroundFAFBED, 안보임|출하일|日|출고시각|업체명|출고지|컷|門|`%
Gui, 1:ListView, lv2
LV_ModifyCol(1, 0) ;
LV_ModifyCol(2, 65)  ;
LV_ModifyCol(3, 30)  ;
...
Gui, 1:Font, S13 CDefault W300, Malgun Gothic
Gui, 1:Add, ListView, y+0 w610 h300 vlv3 glv3 Grid hwndHLV3 NoSortHdr Sort -Multi BackgroundF9FFF5, 안보임|출하일|日|출고시각|업체명|출고지|컷|門|`%
Gui, 1:ListView, lv3
LV_ModifyCol(1, 0) ;
LV_ModifyCol(2, 65)  ;
LV_ModifyCol(3, 30)  ;
...
Gui, 1:Font, S12 CDefault W300, Malgun Gothic
Gui, 1:Add, ListView, y+0 w610 h230 vlv11 glv11 Grid hwndHLV11 NoSortHdr Sort -Multi SortDesc BackgroundFFF0FA, 안보임|출하일|日|출고시각|업체명|출고지|컷|門|`%
Gui, 1:ListView, lv11
LV_ModifyCol(1, 0) ;
LV_ModifyCol(2, 65)  ;
LV_ModifyCol(3, 30)  ;
...
Gui, 1:Add, ListView, x20 y+0 w250 h590 vlvedsm glvedsm Grid hwndHLVedsm NoSortHdr Sort -Multi SortDesc BackgroundFFFCF3, 안보임|년도|발주일|日|진행률
...
if (countm = 5)
{
	Gui, 1:Show, w1280 h730 x-2823 y511, 신흥목재 전산프로그램
}
else
{
...
Loop % LV_GetCount()
{
	...
	else if (RetrievedText = ""){
	{
		...
		{
		CLV3.Cell(A_Index, 8, 0xF9FFF5, 0xFFFFFF)
		}
	}
...
Loop % LV_GetCount()
{
	...
	else if (RetrievedText = "")
	{
		...
		{
		CLV11.Cell(A_Index, 8, 0xFFF0FA, 0xFFFFFF)
		}
	}
	else
	{
...
Gui, %상태%:Add, Picture, x250 y180 W806 H430 vpic3, %A_WorkingDir%\exit.png
GuiControl, hide, pic2
GuiControl, hide, pic3
gui, %상태%:Add, progress, smooth x250 y145 w428 h35 cFF9900 vclosepg, 0
GuiControl, hide, closepg
	...
	Loop % LV_GetCount()
	{
		...
		if (RetrievedText = "◎"){
		  ;~ LV_GetNext(StartingRowNumber, RowType)
			CLV7.ROW(A_Index, 0xff1966, 0xffffff)
		}
		else if (RetrievedText = "★"){
		  ;~ LV_GetNext(StartingRowNumber, RowType)
...
리스트로저장:
SoundPlay, %A_WorkingDir%\sound\효과음201.wav
GuiControl, show, pic2
순수포멧 := "Y"
저장포멧 := "Y.CSV"
변환결과 := "[Ⅰ. 출고 리스트 업]"
...
준비중으로저장:
SoundPlay, %A_WorkingDir%\sound\효과음202.wav
GuiControl, show, pic2
순수포멧 := "X"
저장포멧 := "X.CSV"
변환결과 := "준비중으로저장"
...
준비완료로저장:
SoundPlay, %A_WorkingDir%\sound\효과음203.wav
GuiControl, show, pic2
순수포멧 := "Z"
저장포멧 := "Z.CSV"
변환결과 := "준비완료로저장"
...
완료출하로저장:
SoundPlay, %A_WorkingDir%\sound\효과음204.wav
GuiControl, show, pic2
순수포멧 := "W"
저장포멧 := "W.CSV"
변환결과 := "완료(출고)로저장"
...
;~ if (순수포멧 = "Y")
;~ {
	;~ SoundPlay, %A_WorkingDir%\sound\효과음102.mp3
;~ }
;~ else if (순수포멧 = "X")
;~ {
	;~ SoundPlay, %A_WorkingDir%\sound\효과음101.mp3
;~ }
;~ else if (순수포멧 = "Z")
;~ {
	;~ SoundPlay, %A_WorkingDir%\sound\효과음202.wav
...
Gui, Submit, NoHide
		...
		GuiControl,, closepg, +10
GuiControl, show, pic3
GuiControl, show, closepg
GuiControl, hide, 상태close
GuiControl, hide, 저장1
GuiControl, hide, 카메라1
GuiControl, hide, 리스트로저장
GuiControl, hide, 준비중으로저장
GuiControl, hide, 준비완료로저장
GuiControl, hide, 완료출하로저장

		GuiControl,, closepg, +30
...
if ((contain = "\\192.168.0.1\hdd2\일정표 최근\일정표220730\매입\") or (contain = "\\192.168.0.1\일정표 최근\일정표220730\매입\") )
{
