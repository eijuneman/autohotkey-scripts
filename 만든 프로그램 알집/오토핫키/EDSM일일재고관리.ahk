...
path1 = % A_ScriptDir "\EDSM\어제판매리스트.xlsx"
path2 = % A_ScriptDir "\EDSM\오늘이후판매리스트.xlsx"

FileCopy, % A_ScriptDir "\어제판매리스트.xlsx", %path1%, 1
Sleep,500
FileCopy, % A_ScriptDir "\오늘이후판매리스트.xlsx", %path2%, 1
Sleep,2000
...
Sleep,100
	...
	ControlClick, EVA_ChildWindow1, 카카오톡 ,,,, x36 y90
	Sleep,300
			;~ GuiControl,, myprogress, +10
		WinWait, 카카오톡,
	IfWinNotActive, 카카오톡, , WinActivate, 카카오톡,
...
X1.sheets("sheet1").Range("S:S").select
X1.Selection.copy
...
FormatTime, e1, , ▶ yyyy년 MM월 dd일 dddd  시점재고 파악해주세요 ◀
...
체크상태 = 체크 중
PATH3 := % "\\192.168.0.1\hdd2\일정표 최근\일정표220730\일일재고관리\" 오늘늘짜 ".CSV"

파일이름 := "A" 오늘날짜 "_B" 체크상태


X1.ActiveWorkbook.Save
X1.ActiveWorkbook.SaveAs("\\192.168.0.1\hdd2\일정표 최근\일정표220730\일일재고관리\" . 파일이름, 62)
Sleep, 1000
