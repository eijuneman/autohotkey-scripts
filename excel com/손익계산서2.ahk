...
손익계산날짜 :=	"기간 : " 시작1 "년" 시작2 "월"    ;  손익계산서 제목 부분
...
루틴1:   ;  비용처리 수입지출 내역 확보
{
	...
	ControlClick, TAdvToolBar4, ahk_class TfmMain,,,,x23 y23
	Sleep, 2000
...
루틴2:   ;  매입금액 외상장부 내역 확보
{
	...
	Sleep, 2000
	ControlClick, TAdvToolBar3, ahk_class TfmMain,,,,x282 y13
...
루틴3:   ;  매출 부분    일자전표관리 서  내역 확보
{
	...
	ControlSend, TRzDateTimeEdit2, {Del 20}%시작%, ahk_class TfmMain
	ControlSend, TRzDateTimeEdit1, {Del 20}%끝%, ahk_class TfmMain
	ControlClick, TAdvToolBar3, ahk_class TfmMain,,,,x49 y14
	Sleep, 5000
...
;~ x2Worksheet.Range("D:R").delete     ; 제대로 업데이트 전 로우데이터 양식
;~ x2Worksheet.Range("A:B").delete


x2Worksheet.Range("X:AE").delete
x2Worksheet.Range("D:U").delete
x2Worksheet.Range("A:B").delete
;~ x2Worksheet.insert("B:B").delete
...
x2Workbook.sheets("매입로우데이터").Application.CutCopyMode := False ; 복사모드 해제



x2Workbook.sheets("매입로우데이터").Range("A2:F" 매입행).copy
Sleep, 100
xlWorkbook.sheets("손익계산서").Select
xlWorkbook.sheets("손익계산서").Range("D8").Select
xlWorkbook.sheets("손익계산서").Paste
...
;~ xlWorkbook.sheets("손익계산서").Range("D8").PasteSpecial(-4163)  ;                                         <<<<<<<<<<<<<<<<<<<<<<<<<<<<      계속 문제 되는 구간
...
;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;
    ...
    xlWorkbook.sheets("손익계산서").Range("A3").value := 손익계산날짜   ; 문서 위에 있음
    ...
    xlWorkbook.sheets("손익계산서").Range("B:B").HorizontalAlignment := -4108  ; (C) 중앙
    xlWorkbook.sheets("손익계산서").Range("E:E").HorizontalAlignment := -4108  ; (C) 중앙

    xlWorkbook.sheets("손익계산서").Range("7:37").RowHeight := 20
    xlWorkbook.sheets("손익계산서").Range("38:40").RowHeight := 25
...
;~ xl.ActiveWindow.View := -2    ; -2는 xlPageBreakPreview를 의미

타이틀범위 := "A1:F" 타이틀행

xlWorkbook.sheets("손익계산서").PageSetup.PrintArea := 타이틀범위



;~ xl.Application.CutcopyMode := 0
;~ xl.PrintCommunication := 0
;~ xlWorkbook.Sheets("손익계산서").Pagesetup.zoom := ComObj(0xB,0)
;~ xlWorkbook.Sheets("손익계산서").Pagesetup.FittopagesWide := 1
;~ xlWorkbook.Sheets("손익계산서").Pagesetup.FittopagesTall := 0
...
MsgBox, 262192, 알림, 손익계산서 작성을 완료 했습니다`n시작 : %시작%`n끝 : %끝%
