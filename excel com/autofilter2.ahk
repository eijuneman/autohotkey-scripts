...
Filename := A_Desktop . "\복사본 비용처리2.xlsx"
;~ FileDelete, % Filename
시작 = 2024-07-01
끝 = 2024-07-31
...
;~ {
	...
	;~ {
		try { ; 이미 활성화된 Excel 인스턴스 가져오기
			...
			;~ Global 엑셀03 ; 중요함
			엑셀03 := ComObjActive("Excel.Application")
			엑셀03.Visible := true ; 엑셀 창을 표시하려면 true로 설정
...
ws.Range("H:T").DELETE ; 선택 시트의 A~Z열 복사
		...
		Sleep, 2000
		엑셀03.ActiveWorkbook.Saveas(Filename)
...
; 엑셀 파일 열기
xlWorkbook := xl.Workbooks.Open(Filename)
...
return
	...
	xlWorkbook.sheets("손익계산서").Range("F6").value := 상품매출원가
	xlWorkbook.sheets("손익계산서").Range("I6").value := 상품매출원가
	xlWorkbook.sheets("손익계산서").Range("F5").value := 상품매출원가
	xlWorkbook.sheets("손익계산서").Range("I5").value := 상품매출원가
...
xlWorkbook.sheets("손익계산서").Range("I" 20행).value := 매출총이익
xlWorkbook.sheets("손익계산서").Range("H" 20행).value := 매출총이익

xlWorkbook.sheets("손익계산서").font.size := 10
return
