...
ExampleURL	:=	"https://www.hometax.go.kr/websquare/websquare.wq?w2xPath=/ui/pp/index_pp.xml&tmIdx=&tm2lIdx=&tm3lIdx="
ExampleURL2	:=	 "https://www.hometax.go.kr/websquare/websquare.wq?w2xPath=/ui/pp/index_pp.xml&tmIdx=0&tm2lIdx=100907&tm3lIdx=" ; 실전
...
;~ Var:=pwb.document.getElementsByClassName("classnameXXX").item[0].Value ;Get classname and Array value

While (! Element:=pwb.document.getElementByID("ftGroup904564563")) ;Make sure element exists before moving forward
	Sleep, 2000
;~ MsgBox, 기달
;~ pwb.document.getElementById("user_id").value := "신흥목재"
;~ pwb.document.getElementById("user_pw").value := "48342"

;~ pwb.document.getElementById("textbox915")[0].Click()
;~ pwb.document.getElementByID("hdTextbox548").Click()
pwb.document.getElementByID("hdMyNTS").Click()
	Sleep, 2000

;~ IELoad(pwb) ; sleep
;~ While (! Element:=pwb.document.getElementByID("ftGroup904564563")) ;Make sure element exists before moving forward
	;~ Sleep, 2000
;~ pwb.document.getElementByID("A208").Click()


IELoad(pwb) ; sleep
While (! Element:=pwb.document.getElementByID("ftGroup904564563")) ;Make sure element exists before moving forward
	Sleep, 2000
pwb.document.getElementByID("anchor22")[].Click()
...
Loop{
	if(FileExist(A_ScriptDir "\우딘\우딘.csv"))
	{
		...
		xl.sheets("우딘").Range("A:A").select
		xl.Selection.copy
		xl.sheets("우딘").Range("M:M").select
		xl.Selection.pastespecial


		xl.sheets("우딘").Range("B:B").select
		xl.Selection.copy
	...
	xl.sheets("우딘").Range("F:F").select
	xl.Selection.copy
	xl.sheets("우딘").Range("D:D").select
	xl.Selection.pastespecial(-4163)


	xl.sheets("우딘").Range("G:G").select
	xl.Selection.copy
	xl.sheets("우딘").Range("E:E").select
	xl.Selection.pastespecial(-4163)


	xl.sheets("우딘").Range("F:N").select
	xl.Selection.DELETE
	...
	{
	공식셀C := % "C" 행시작
	xl.sheets("우딘").range(공식셀C).value := "ea"
	행시작 --
	}
